...
use eyeball_im::{ObservableVector2Entry, ObservableVector2WriteGuard};
...
use super::{
    ...
    inner::{TimelineInnerMetadata, TimelineInnerStateWriteGuard},
    item::timeline_item,
    polls::PollState,
...
pub(super) struct TimelineEventHandler<'a, 'o> {
    items: &'a mut ObservableVector2WriteGuard<'o, Arc<TimelineItem>>,
...
impl<'a, 'o> TimelineEventHandler<'a, 'o> {
    pub(super) fn new(
        state: &'a mut TimelineInnerStateWriteGuard<'o>,
        ctx: TimelineEventContext,
        track_read_receipts: bool,
    ) -> Self {
        let TimelineInnerStateWriteGuard { items, meta } = state;
...
pub(crate) fn update_read_marker(
    items: &mut ObservableVector2WriteGuard<'_, Arc<TimelineItem>>,
