...
use eyeball_im::{ObservableVector2, ObservableVector2Entry, ObservableVector2WriteGuard};
...
pub(in crate::timeline) struct TimelineInnerState {
    pub items: ObservableVector2<Arc<TimelineItem>>,
...
impl TimelineInnerState {
    pub(super) fn new(room_version: RoomVersionId) -> Self {
        Self {
            ...
            // small enough.
            items: ObservableVector2::with_capacity(32),
    ...
    pub(super) fn handle_local_event(
        ...
        TimelineEventHandler::new(&mut self.write(), ctx, settings.track_read_receipts)
    ...
    pub(super) fn update_timeline_reaction(
        ...
        self.items.write().set(idx, item);

        Ok(())
    }

    pub fn write(&mut self) -> TimelineInnerStateWriteGuard<'_> {
        TimelineInnerStateWriteGuard { items: self.items.write(), meta: &mut self.meta }
    }
}

pub(in crate::timeline) struct TimelineInnerStateWriteGuard<'a> {
    pub items: ObservableVector2WriteGuard<'a, Arc<TimelineItem>>,
...
impl TimelineInnerStateWriteGuard<'_> {
    ...
    pub(super) fn clear(&mut self) {
        ...
        if self.items.iter().any(|item| item.is_local_echo()) {
            ...
            self.items.for_each(|entry| {
                if entry.is_remote_event() || entry.is_read_marker() {
                    ObservableVector2Entry::remove(entry);
                }
            });
        ...
        self.meta.event_should_update_fully_read_marker = false;
    }
