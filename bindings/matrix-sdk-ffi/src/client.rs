...
impl Client {
    ...
    pub fn restore_session(&self, session: Session) -> Result<(), ClientError> {
        ...
        let auth_session = self.auth_session_from_session(session)?;
...
impl Client {
    ...
    fn retrieve_session(&self) -> Result<SessionTokens, String> {
        ...
        let auth_session = self.auth_session_from_session(session).map_err(|e| e.to_string())?;
        match auth_session {
    ...
    // TODO: Implement From/Into here for both directions.
    /// Creates an SDK `AuthSession` from an FFI `Session`.
    fn auth_session_from_session(&self, session: Session) -> Result<AuthSession, ClientError> {
        let Session {
            access_token,
            refresh_token,
            user_id,
